...
void SaveToFile::SetListener(WebcamListener* pListener)
{
  mpWebcamListener = pListener;
}

WebcamListener* SaveToFile::Listener() const
{
  return mpWebcamListener;
}
...
bool SaveToFile::HandleVideo(const void* inpData)
{
  try {
    ...
    static const int cWarningSamples = 100;
    switch (mState.load()) {
    case Idle:
      break;
    case Recording:
...
void SaveToFile::OnRecordVideo(const void* inpData)
{
  ...
  if (mpWebcamListener)
    mpWebcamListener.load()->OnWebcamFrameNumber(frameNumber);
