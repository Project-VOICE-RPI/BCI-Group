...
struct WebcamThread::Private
{
    ...
    WebcamListener* mpListener = nullptr;
...
bool WebcamThread::Start(int cameraIndex, const WebcamController::Parameters& config, WebcamListener* pClient)
{
    if (p->mThread.Running())
        Stop();
    p->mCameraIndex = cameraIndex;
    p->mConfig = config;
    p->mpListener = pClient;
    p->mThread.Start();
    std::unique_lock<std::mutex> lock(p->mMutex);
  ...
  try {
    ...
    while (!mPreflight && mState != terminating && ok) {
        ...
        if (ok) {
            ...
            // measure average frame duration to make sure frame rate is correct
            if (frameCount++ == 0) {
                begin = TimeUtils::MonotonicTime();
            }
            if (frameCount == cMeasureFrames + 1) {
                ...
                if (ratio < 0.9 || ratio > 1.1) {
                    ...
                    mpListener->OnWebcamMessage(oss.str());
                }
            }
            ...
            if (mpListener) {
                mpListener->OnWebcamFrameData(width, height, rgbData.data());
            }
        }
    }
    if (!mpSource->Error().empty())
        throw std_runtime_error << mpSource->Error();
    ...
    lock.unlock();
    if (mpListener) {
        mpListener->OnWebcamFrameData(0, 0, nullptr);
    }
    mConditionVariable.notify_one();
  }
}
