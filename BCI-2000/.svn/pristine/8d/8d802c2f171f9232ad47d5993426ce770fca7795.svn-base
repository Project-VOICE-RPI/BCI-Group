...
{
  HRSucceedOrThrow hr = ::MFCreateVideoRendererActivate(NULL, mpRendererActivate.Assignee());
  mDoneEvent = ::CreateEventA(nullptr, FALSE, FALSE, nullptr);
  hr = ::MFCreateMediaSession(nullptr, mpMediaSession.Assignee());
...
void WebcamPipeline::OnMediaError(HRESULT hr)
{
  ...
  if (mWindowHandle)
    ::SendMessageA(mWindowHandle, WM_DESTROY, 0, 0);
}
...
void WebcamPipeline::SetDesiredVideoMediaType(const MFVideoMediaType& type)
{
  std::lock_guard<std::mutex> lock(mMutex);
  mDesiredVideoMediaType = type;
}

const MFVideoMediaType& WebcamPipeline::ActualVideoMediaType() const
{
  std::lock_guard<std::mutex> lock(mMutex);
  return mActualVideoMediaType;
}

void WebcamPipeline::SetDesiredAudioMediaType(const MFAudioMediaType& type)
{
  std::lock_guard<std::mutex> lock(mMutex);
  mDesiredAudioMediaType = type;
}

const MFAudioMediaType& WebcamPipeline::ActualAudioMediaType() const
{
  std::lock_guard<std::mutex> lock(mMutex);
  return mActualAudioMediaType;
}

void WebcamPipeline::SetCameraMediaSource(com::Ptr<IMFMediaSource> inpSource, const std::string& inName)
{
    std::lock_guard<std::mutex> lock(mMutex);
    mpVideoSource = inpSource;
    mCameraName = inName;
}
...
void WebcamPipeline::WaitForDone(int timeoutMs)
{
  ...
  std::lock_guard<std::mutex> lock(mMutex);
  if (!mError.empty())
      throw std_runtime_error << mError;
}
...
void WebcamPipeline::ChooseVideoType(IMFMediaTypeHandler* pHandler)
{
  ...
  std::vector<MFVideoMediaType> formats;
  ...
  auto j = std::min_element(formats.begin(), formats.end(),
    [this](const MFVideoMediaType& a, const MFVideoMediaType& b)
    {
...
void WebcamPipeline::ChooseAudioType(IMFMediaTypeHandler* pHandler)
{
    ...
    DWORD count = 0;
    hr = pHandler->GetMediaTypeCount(&count);
    std::vector<MFAudioMediaType> formats;
    for (int i = 0; i < count; ++i) {
        com::Ptr<IMFMediaType> pMediaType;
        hr = pHandler->GetMediaTypeByIndex(i, pMediaType.Assignee());
        formats.push_back(pMediaType);
    ...
    auto j = std::min_element(formats.begin(), formats.end(),
        [this](const MFAudioMediaType& a, const MFAudioMediaType& b)
        {
