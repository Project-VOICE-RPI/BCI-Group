  ...
  DWORD dwFlags)
{
  ...
  for (auto& pAvailableType : mInputAvailableTypes[dwInputStreamID]) {
    if (MFVideoMediaType::IsVideo(pType) && MFVideoMediaType::IsVideo(pAvailableType))
      isCompatible = isCompatible || MFVideoMediaType(pType).IsCompatible(MFVideoMediaType(pAvailableType));
    else if(MFAudioMediaType::IsAudio(pType) && MFAudioMediaType::IsAudio(pAvailableType))
      isCompatible = isCompatible || MFAudioMediaType(pType).IsCompatible(MFAudioMediaType(pAvailableType));
  }
  if (!isCompatible)
  ...
  DWORD dwFlags)
{
  ...
  if (MFVideoMediaType::IsVideo(pType) && MFVideoMediaType(pType).IsCompatible(MFVideoMediaType(mInputTypes[dwOutputStreamID])))
  {
    mOutputActive[dwOutputStreamID] = true;
    return S_OK;
  }
  if (MFAudioMediaType::IsAudio(pType) && MFAudioMediaType(pType).IsCompatible(MFAudioMediaType(mInputTypes[dwOutputStreamID])))
  {
    mOutputActive[dwOutputStreamID] = true;
    return S_OK;
  }
